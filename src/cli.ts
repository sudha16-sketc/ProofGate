...
async function cmdDemo(network: any, networkConfig: any, contractAddress: string, seed: string) {
  ...
  console.log('1/5 — Admin activates a compliance policy (minAge ≥ 18, KYC ≥ 2)...');
  ...
  console.log('2/5 — Admin registers a trusted KYC issuer (public: issuer id hash)...');
  const tx1 = await found.callTx.registerIssuer(issuer.pubX, issuer.pubY, new Uint8Array(32));
  printPrivacyBanner();
  console.log(`    ✅ txId=${tx1.public.txId}\n`);

  console.log('3/5 — User registers a credential (verifies issuer signature, age ≥ minAge, jurisdiction)...');
